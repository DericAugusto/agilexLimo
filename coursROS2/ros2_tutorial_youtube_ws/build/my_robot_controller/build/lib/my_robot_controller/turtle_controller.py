...
"""
This script creates a ROS2 node that manipulates a turtle's movement in the 
...
Classes:
--------
TurtleController - A ROS2 node that manipulates a turtle's movement.
...
the node has started, providing visibility into the node's lifecycle.

pose_callback - This method is triggered whenever a new pose message is received
from the turtle's pose topic. It evaluates the turtle's current position. If the
turtle is outside a defined square (x > 9.0 or x < 2.0 or y > 9.0 or y < 2.0), 
it publishes a velocity command to make the turtle turn right and move forward, 
effectively guiding it back towards the defined square. If the turtle is inside 
the square, it publishes a velocity command to make the turtle move straight 
ahead at a faster speed, allowing it to roam freely within the defined area.
"""
